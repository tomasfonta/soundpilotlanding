...
3. Confirm the page is the intended signed-in account and the composer is usable.
   ...
   - If the visible page shows `免费版`, a free account, a non-Pro account, or no usable Pro model route, stop and ask the user to switch/open the intended logged-in ChatGPT account in Chrome. Do not continue with a free-tier route when Pro was requested.
...
10. Extract the answer from the real page.
    ...
    - Verify the copied text is the assistant answer, not the user prompt. If copied text lacks the answer sections, starts with the submitted prompt, or is much shorter than the visible response, extraction failed; try DOM extraction next.
    - If no reliable copy button is available, extract from the visible DOM/accessibility tree. Prefer `[data-message-author-role="assistant"]` when the ChatGPT DOM exposes it. With Computer Use, use visible OCR and tab screenshots for uncertain content.
    - Verify the copied/extracted text contains the unique completion marker and matches the visible beginning/end. If only an accessibility-tree reconstruction is possible, preserve the key sections and link to the ChatGPT conversation URL so the user can verify original formatting.
...
- If Chrome plugin control tools are unavailable, use Computer Use fallback when it can operate the real visible ChatGPT page. Do not produce a substitute research answer from non-ChatGPT sources or API calls.
...
- If extraction is uncertain, try the copy button, DOM extraction, and accessibility-tree extraction in that order. If still uncertain, ask the user to copy the response from ChatGPT and compare results.
